...
void process_readings(float ppm25, float ppm10)
{
	...
	{
		pub_pm25Average.addToAverage(ppm25);
		pub_pm10Average.addToAverage(ppm10);

		if (pub_pm10Average.averageReady())
		{
			pub_pm10Average.getAverage(&pub_avg_ppm_10);
			pub_pm25Average.getAverage(&pub_avg_ppm_25);
...
void setup_sensor()
{
	...
	SDSSensorSerial.begin(9600, SERIAL_8N1, 17, 13);  // badud, mode, rx, tx
	reset_message();
