...
void do_splash_screen_bottom_line(JsonObject &root, char *resultBuffer)
{
	...
	if (reply == WORKED_OK)
	{
		...
		if (!option)
		{
			build_text_value_command_reply(WORKED_OK, settings.splash_screen_bottom_line, root, resultBuffer);
			return;
		}
	...
	if (reply == WORKED_OK)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					reply = INVALID_GPS_STATUS_SETTING;
				}
			}
		}
	}
	...
	if (reply == WORKED_OK)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					reply = INVALID_RTC_STATUS_SETTING;
				}
			}
		}
	}
...
void reset_settings()
{
	...
	// a pre-set WiFi location
	strcpy(settings.wifiSettings[0].wifiSsid, "CHtestnetwork");
	strcpy(settings.wifiSettings[0].wifiPassword, "CHtest123!");

	// clear the rest of the settings
	for (int i = 1; i < NO_OF_WIFI_SETTINGS; i++)
	{
...
void act_onBinary_command(u1_t *buffer, int length, char *displayBuffer, int displayBufferLength)
{
	...
	if (length == 0)
	{
		snprintf(displayBuffer, displayBufferLength, "Missing command");
		return;
	}
